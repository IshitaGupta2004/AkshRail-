...
def login_page():
    ...
    # --- STYLE FOR LOGIN PAGE (LIGHT & DARK MODE) ---
    st.markdown("""
    <style>
    /* --- Base Styles --- */
    .stButton>button {
...
def main_app():
    ...
    # This is the most reliable method. It injects a script that runs
    # on every page interaction and forces the main window to scroll to the top.
    st.markdown(
        """
        <script>
            (function() {
                try {
                    // Try to scroll the parent window, which is the main browser window
                    window.parent.scrollTo(0, 0);
                } catch (e) {
                    // Fallback for environments where parent access is restricted
                    window.scrollTo(0, 0);
                }
            })();
        </script>
    ...
    # Custom CSS for better aesthetics and new card styles
    st.markdown("""
    <style>
        /* --- 1. THEME VARIABLES --- */
        :root {
        ...
        @media (prefers-color-scheme: dark) {
            :root {
                --primary-color: #0078D4;
                --primary-color-hover: #1085D6;
                --header-color: #E1E1E1;
                --text-color: #D1D1D1;
                --card-bg: #1E2A38;
                --card-border: #4A5568;
                --card-shadow: 0 4px 6px rgba(0,0,0,0.2);
                --sidebar-bg: linear-gradient(to bottom, #1E2A38, #2c3e50);
                --success-bg: #155724; --success-text: #d4edda;
                --warning-bg: #856404; --warning-text: #fff3cd;
                --error-bg: #721c24; --error-text: #f8d7da;
                --info-bg: #0c5460; --info-text: #d1ecf1;
            }
        }
        ...
        /* Role-Based Summary on Dashboard */
        .custom-info-alert {
        ...
        .deadline-card-error { background-color: var(--error-bg); color: var(--error-text); }
        .deadline-card-warning { background-color: var(--warning-bg); color: var(--warning-text); }
        .deadline-card-info { background-color: var(--info-bg); color: var(--info-text); }
        .deadline-card-title { font-size: 1.1em; font-weight: bold; margin-bottom: 10px; }
        .deadline-card-days { font-size: 2.8em; font-weight: bold; }
        .deadline-card-text { font-size: 1em; margin-bottom: 10px; }
        .deadline-card-date { font-size: 0.9em; }

        /* Score Cards on Analytics */
        .score-card {
        ...
        .score-card-label { font-size: 1em; color: var(--text-color); font-weight: bold; margin-bottom: 5px;}
        .score-card-value { font-size: 2.2em; font-weight: bold; color: var(--header-color); }
        .score-card-delta { font-size: 0.9em; }
        .score-card-delta-positive { color: #28a745; }
        .score-card-delta-negative { color: #dc3545; }
    ...
    menu = st.sidebar.radio("Go to:", ["Home", "Dashboard", "Upload", "Search", "Summary", "Analytics", "About"])
    st.sidebar.markdown("---")
    ...
    summary_page_lottie = load_lottie_url("https://assets5.lottiefiles.com/packages/lf20_bcjfi9q9.json") # Summary
    ...
    if menu == "Home":
        ...
        with col_text:
            ...
            st.markdown('<div class="custom-info-alert">Our mission: To transform document overload into a streamlined, efficient information hub.</div>', unsafe_allow_html=True)

        st.markdown("---")
        st.subheader(TEXT["core_capabilities"])
        st.caption("An overview of the key AI-driven features that power AkshRail.")
        
        col_cap1, col_cap2, col_cap3 = st.columns(3)
        with col_cap1:
            st.markdown(f"<h4>{TEXT['ocr_title']}</h4>", unsafe_allow_html=True)
            st.markdown(TEXT['ocr_desc'])
        with col_cap2:
            st.markdown(f"<h4>{TEXT['nlp_title']}</h4>", unsafe_allow_html=True)
            st.markdown(TEXT['nlp_desc'])
        with col_cap3:
            st.markdown(f"<h4>{TEXT['search_title']}</h4>", unsafe_allow_html=True)
    ...
    # ---------- DASHBOARD ----------
        ...
        st.caption("Your main control center: Get an overview of document activity, pending tasks, and system health.")
        
        # --- Critical Alerts Banner ---
        st.markdown(
            """
            <div class="custom-error-alert" style="margin-top: 1em;">
            <strong>🚨 CRITICAL ALERT</strong><br>
            Unexpected maintenance on Line 2 scheduled for tonight. Delays expected.
            </div>
            """, unsafe_allow_html=True
        )
        st.markdown("---")
        ...
        st.caption("A personalized feed of updates and tasks tailored to your role.")
        role = st.session_state.get('role', 'Guest')
        
        if role == "Senior Manager":
            st.markdown(f'<div class="custom-info-alert"><strong>For Senior Manager:</strong> High-level overview of monthly document submissions and pending approvals. The <strong>Analytics</strong> tab shows key performance indicators for department efficiency.</div>', unsafe_allow_html=True)
        elif role == "Station Controller":
            st.markdown(f'<div class="custom-info-alert"><strong>For Station Controller:</strong> Real-time incident reports from Line 1 and Line 2 are available on the <strong>Search</strong> page. A new station circular `DOC-4512` requires acknowledgment.</div>', unsafe_allow_html=True)
        elif role == "Operations Officer":
            st.markdown(f'<div class="custom-info-alert"><strong>For Operations Officer:</strong> Daily operational logs have been auto-summarized. Please review the \'Shift Handover Summary\' from the last 24 hours. The system has flagged a potential scheduling conflict for next week.</div>', unsafe_allow_html=True)
        elif role == "Maintenance Engineer":
            st.markdown(f'<div class="custom-info-alert"><strong>For Maintenance Engineer:</strong> Recent maintenance reports and technical drawings have been uploaded. Check the <strong>Search</strong> page for \'Safety Protocol Update DOC-1150\'.</div>', unsafe_allow_html=True)
        elif role == "Administrative Staff":
            st.markdown(f'<div class="custom-info-alert"><strong>For Administrative Staff:</strong> There are 15 new invoices awaiting processing. A reminder for upcoming contract renewals is displayed in the <strong>Compliance Deadlines</strong> section below.</div>', unsafe_allow_html=True)
        else: # Guest and other roles
            st.warning(
                "You are logged in as a Guest. You have read-only access to public documents. "
                "Please contact an administrator for elevated privileges."
            )
        st.markdown("---")

        # --- Compliance Deadline Countdowns ---
        st.subheader("Upcoming Compliance Deadlines")
        st.caption("Track important deadlines for reports, audits, and renewals at a glance.")
        ...
        cols = st.columns(len(deadline_items))

        for i, col in enumerate(cols):
            with col:
                task, deadline = deadline_items[i]
                days_left = (deadline - today).days
                
                card_class = "deadline-card-info"
                if days_left < 10:
                    card_class = "deadline-card-error"
                elif days_left < 30:
                    card_class = "deadline-card-warning"

                st.markdown(f"""
                <div class="deadline-card {card_class}">
                    <div class="deadline-card-title">{task}</div>
                    <div class="deadline-card-days">{days_left}</div>
                    <div class="deadline-card-text">Days Left</div>
                    <div class="deadline-card-date">Due: {deadline.strftime('%b %d, %Y')}</div>
                </div>
                """, unsafe_allow_html=True)
        ...
        st.caption("AI-detected potential duplicate documents that may require your review.")
        st.warning("""
        **Potential Duplicates Found:**
        ...
        st.caption("Key metrics showing the overall health and usage of the document management system.")
        ...
        st.caption("A live feed of the most recently uploaded and modified documents across the system.")
        activity_data = {
        ...
        st.caption("Visualize document upload frequency across different departments over the past week.")
        ...
        with st.expander("Frequently Asked Questions (FAQ)"):
            st.markdown("**What is the 'Your Role-Based Summary' section?**")
            st.write("""
            ...
            """)
            st.markdown("**Why do I see a critical alert at the top of my dashboard?**")
            st.write("""
            ...
            """)
            st.markdown("**How are the compliance deadlines calculated?**")
            st.write("""
            ...
            """)
            st.markdown("**What does a 'Duplicate File Alert' mean?**")
            st.write("""
            ...
            """)
            st.markdown("**How should I interpret the Department Activity Heatmap?**")
            st.write("""
            ...
            """)
            st.markdown("**Can I customize the dashboard view?**")
            st.write("""
    ...
    # ---------- UPLOAD ----------
        ...
        st.caption("Use this form to add new documents to the system. Once uploaded, they will be automatically processed and indexed for searching.")
            ...
            with st.form("document_upload_form"):
                ...
                if submit_button:
                    if uploaded_file is not None:
                        ...
                        st.caption("Review the text extracted from the document. You can make corrections here before it is saved.")
                        st.text_area(
        ...
        with st.expander("Frequently Asked Questions (FAQ)"):
            st.markdown("**How does the OCR Preview work?**")
            st.write("""
            ...
            """)
            st.markdown("**Is the auto-classification of documents always accurate?**")
            st.write("""
            ...
            """)
            st.markdown("**What do the different confidentiality levels mean?**")
            st.write("""
            ...
            """)
            st.markdown("**Can I upload multiple files at once?**")
            st.write("""
    ...
    # ---------- SEARCH ----------
        ...
        st.caption("Use keywords, document IDs, or natural language queries to find any document instantly. Refine your search using filters.")
        ...
        if search_button and query:
            ...
            st.caption("Documents matching your query, ranked by relevance.")
            ...
            if not df_results.empty:
                ...
                st.caption("AI-powered recommendations for documents related to your search.")
                st.info("""
        ...
        with st.expander("Frequently Asked Questions (FAQ)"):
            st.markdown("**How does Voice Search work?**")
            st.write("""
            ...
            """)
            st.markdown("**What are Contextual Recommendations?**")
            st.write("""
            ...
            """)
            st.markdown("**Where do my Pinned Documents go?**")
            st.write("""
            ...
            """)
            st.markdown("**Can I use advanced search operators?**")
            st.write("""
    ...
    # ---------- SUMMARY PAGE ----------
        ...
        st.subheader("📄 Document Quick Summary")
        st.caption("Upload a new document or select an existing one to generate a detailed, AI-powered summary.")
        if summary_page_lottie:
            st_lottie(summary_page_lottie, height=150, key="summary_lottie")

        tab1, tab2 = st.tabs(["Upload a Document", "Select Existing Document"])
        document_source = None
        document_name = None

        with tab1:
            uploaded_file = st.file_uploader("Choose a document to summarize", type=["pdf", "docx", "txt"])
            if uploaded_file:
                document_source = "upload"
                document_name = uploaded_file.name

        with tab2:
            existing_docs = ["<Select a document>", "DOC-1023: Maintenance Schedule Q3", "DOC-1150: Safety Protocol Update", "DOC-0998: Board Meeting Minutes"]
            selected_doc = st.selectbox("Choose an existing document", existing_docs)
            if selected_doc != "<Select a document>":
                document_source = "existing"
                document_name = selected_doc

        if st.button("✨ Generate Summary", disabled=(document_source is None)):
            with st.spinner(f"Analyzing '{document_name}'... This may take a moment."):
                import time
                time.sleep(3) 

            st.success(f"Summary Generated for **{document_name}**")
            ...
            with st.expander("View Detailed Summary", expanded=True):
                st.markdown(
                    ...
                    <div class="summary-section">
                        <h4>Key Points</h4>
                        <p style="font-size: 0.9em; margin-top: -15px;"><i>The most important sentences and takeaways from the document.</i></p>
                        <ul>
                            <li>Routine maintenance on Line 2 was completed with a 5% budget overrun.</li>
                            <li>A new safety protocol (DOC-1150) regarding emergency evacuations has been implemented.</li>
                            <li>Quarterly financial review shows a 7% increase in operational costs, primarily due to energy prices.</li>
                        </ul>
                    </div>
                    """, unsafe_allow_html=True
                )
                ...
                st.markdown(
                    ...
                    <div class="summary-section">
                        ...
                        <p style="font-size: 0.9em; margin-top: -15px;"><i>People, organizations, and places mentioned in the text.</i></p>
                    </div>
                    """, unsafe_allow_html=True
                )
                col_ent1, col_ent2, col_ent3 = st.columns(3)
                with col_ent1:
                    st.info("**People:** Mr. Sharma, Ms. Priya Singh")
                with col_ent2:
                    st.warning("**Organizations:** ABC Corp, RailTech Inc.")
                with col_ent3:
                    st.error("**Locations:** Kaloor Station, Aluva Yard")

                st.markdown(
                    """
                    <div class="summary-section">
                        <h4>Identified Action Items</h4>
                        <p style="font-size: 0.9em; margin-top: -15px;"><i>Potential tasks or follow-ups identified by the AI.</i></p>
                        <ul>
                            <li><b>Follow up</b> with ABC Corp regarding invoice #4567.</li>
                            <li><b>Schedule</b> a team-wide briefing on the new safety protocol (DOC-1150).</li>
                            <li><b>Submit</b> the Q3 financial report for final audit by October 15th.</li>
                        </ul>
                    </div>
                    """, unsafe_allow_html=True
                )
                ...
                st.markdown(
                    ...
                    <div class="summary-section">
                        ...
                        <p style="font-size: 0.9em; margin-top: -15px;"><i>The overall tone of the document (Positive, Negative, or Neutral).</i></p>
                    </div>
                    """, unsafe_allow_html=True
                )
                st.metric(label="Document Sentiment", value="Neutral-Negative", delta="-0.15 Score")
                st.progress(35)
        
        st.markdown("---")
        
        with st.expander("Frequently Asked Questions (FAQ)"):
            st.markdown("**How does the summarization work?**")
            st.write("The system uses advanced AI models (like T5 or BART) to read the document's text and identify the most important sentences and concepts. This process, called abstractive summarization, allows the AI to generate new, concise sentences that capture the core meaning of the original document.")
            
            st.markdown("**What are 'Entities Detected'?**")
            st.write("The AI scans the text for specific categories of information, such as names of **People**, **Organizations**, **Locations**, and **Dates**. This feature, known as Named Entity Recognition (NER), helps you quickly see who and what the document is about without reading the whole text.")

            st.markdown("**How are 'Action Items' identified?**")
            st.write("The system is trained to look for sentences with action-oriented language (e.g., 'submit the report,' 'schedule a meeting,' 'follow up with...') to suggest potential tasks or next steps. This helps you quickly identify responsibilities and deadlines.")

            st.markdown("**Is the summary always 100% accurate?**")
            st.write("While the AI is highly accurate, it's designed to be a powerful assistant, not a replacement for human judgment. For critical decisions, it's always best to consult the original document. The summary provides a fast and reliable overview to help you prioritize your work.")

        with st.expander("🛠️ Functions and Technologies on Summary Page"):
            st.write("""
            - **`st.tabs`, `st.file_uploader`, `st.selectbox`**: Used to create an intuitive UI where users can either upload a new document or select an existing one.
            - **Backend Processing (Simulated)**: The **Generate Summary** button would trigger an API call to a backend server where the heavy AI processing occurs.
            - **NLP Models (Hugging Face Transformers)**: The core of this feature is a suite of Natural Language Processing models.
                - **Summarization**: Utilizes pre-trained models like **BART**, **T5**, or **Pegasus** to generate abstractive summaries.
                - **Named Entity Recognition (NER)**: A model (often based on **BERT** or **spaCy**) is used to find and categorize entities like people, organizations, and locations.
                - **Sentiment Analysis**: A text classification model determines if the overall tone of the document is positive, negative, or neutral.
            - **`st.spinner` & `st.expander`**: Provide visual feedback to the user during the summarization process and display the final results in a clean, collapsible container.
            - **Custom CSS**: The `st.markdown` with `<style>` tags is used to create bordered containers (`.summary-section`) for a more organized and readable output.
            """)
    ...
    # ---------- ANALYTICS ----------
        ...
        st.caption("Unlock meaningful insights from your document data. Visualize trends, identify bottlenecks, and monitor system usage.")
        ...
        st.caption("High-level metrics that measure organizational performance and workflow efficiency.")
        col_score, col_latency, col_bottleneck = st.columns(3)

        with col_score:
            st.markdown("""
            <div class="score-card">
                <div class="score-card-label">Compliance Score</div>
                <div class="score-card-value">95.7%</div>
                <div class="score-card-delta score-card-delta-positive">+1.2% vs. last month</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_latency:
            st.markdown("""
            <div class="score-card">
                <div class="score-card-label">Avg. Decision Latency</div>
                <div class="score-card-value">48 Hours</div>
                <div class="score-card-delta score-card-delta-negative">-5 hours vs. last month</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_bottleneck:
            st.markdown("""
            <div class="score-card">
                <div class="score-card-label">Top Bottleneck Dept</div>
                <div class="score-card-value">Finance</div>
                <div class="score-card-delta" style="color: transparent;">-</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col_chart1:
            st.markdown("#### Decision Latency Report")
            st.caption("This chart shows the average time taken from a document's upload to its first review or action, categorized by document type.")
            latency_data = {
        ...
        with col_chart2:
            ...
            st.caption("This chart highlights the average time each department takes to process documents, helping to identify potential workflow bottlenecks.")
            bottleneck_data = {
        ...
        st.caption("See which departments are the most active in uploading documents to the system over the last 30 days.")
        contributors_data = {
        ...
        fig_contributors = px.pie(df_contributors, values='Uploads (Last 30 Days)', names='Department',
                                title="Department Uploads (Last 30 Days)",
                                hole=.4) # Donut chart
        st.plotly_chart(fig_contributors, use_container_width=True)
        
        st.markdown("#### Top 5 Most Searched Keywords")
        st.caption("Discover the most common topics and keywords users are searching for across the entire document repository.")
        keywords_data = {
        ...
        fig_keywords = px.scatter(df_keywords,
                                  x="Search Count", y="Keyword",
                                  title="Most Frequent Search Terms (Dot Plot)",
                                  size="Search Count",
                                  color="Keyword")
        ...
        with st.expander("Frequently Asked Questions (FAQ)"):
            st.markdown("**What is the Contributors Board?**")
            st.write("""
            ...
            """)
            st.markdown("**What is 'Decision Latency'?**")
            st.write("""
            ...
            """)
            st.markdown("**How do you identify a 'Bottleneck' department?**")
            st.write("""
            ...
            """)
            st.markdown("**What does the 'Compliance Score' represent?**")
            st.write("""
            ...
            """)
            st.markdown("**Can I export these charts and reports?**")
            st.write("""
        ...
        with st.expander("🛠️ Functions and Technologies on Analytics Page"):
            ...
            - **Plotly Express (`plotly.express`):** Used for creating interactive and visually rich charts:
                - **Bar Charts:** For comparing categories like latency and bottleneck analysis.
                - **Pie/Donut Charts:** For showing the proportion of uploads by department.
                - **Scatter Plots:** Used to create dot plots for ranked data like top keywords.
